...
The workspace [`.bpmnlintrc`](../.bpmnlintrc) references the plugin by its short
...
`bpmnlint-plugin-custom-rules` package via a `file:` dependency.
...
3. Open `resources/example-process/` as a workspace in VS Code and open
   `lint-demo.bpmn`. The task named `TODO implement me` shows an ❌ overlay, the
