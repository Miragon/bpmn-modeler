/**
 * The deployment form's DOM, as a single template-literal string injected into
 * `#app` by `main.ts` before the forms query their elements by id.
 *
 * This is the **single source** of the form markup. Every host shell (the Vite
 * `index.html`, the VS Code `DeploymentWebviewHtml.ts`, the IntelliJ deployment
 * tool-window shell) ships only an empty `<div id="app"></div>` and lets the
