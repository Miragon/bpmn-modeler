 ...
 * stream — the transport between the IntelliJ host (Kotlin) and this Node core.
 *
 * NDJSON rather than LSP-style `Content-Length` framing because the spike's only
 * goal is to validate the *protocol shape* (host ports ⇄ core), not to ship a
 * production transport: one JSON object per line is the smallest thing that
 * carries requests, responses, and notifications in both directions.
 *
 ...
 * core only *issues* requests for the document port (write/save) and only
 * *receives* notifications (session/webview lifecycle); the class supports the
 * full matrix so the same code serves the host side conceptually too.
 */
