...
import { createBranchWatcher, type BpmnIqBranchWatcher } from "./BpmnIqBranchWatcher";
...
export class BpmnIqBranchSwitchOrchestrator {
    ...
    private async switchToBranch(targetBranch: string, daemonUrl: string): Promise<void> {
        const root = this.activeRoot;
        const prev = this.activeGitInfo;
        if (!root || !prev) return;

        this.vsUI.logInfo(`[bpmn-iq] branch changed: ${prev.branch} -> ${targetBranch}`);

        const folder = workspace.workspaceFolders?.find((f) => f.uri.fsPath === root);
        if (!folder) {
