...
/** Inline shape passed to {@link BpmnIqPort.registerWorkspace}. */
...
export class BpmnIqSyncService implements Disposable {
    ...
    readonly onDidChangeState: Event<BpmnIqSyncSnapshot> = this._onDidChangeState.event;
    ...
    private meta: WorkspaceConfig | null = null;
    ...
    private buildRegisterOptions(opts: StartOptions): RegisterOptions {
     ...
     *
     * No clear-active counterpart exists — the daemon handles session
     * staleness on its own.  Consumers stop calling when there's nothing
     * to focus.
     */
    async setSessionActive(active: SessionActive): Promise<void> {
        if (!this.port) return;
        try {
    ...
    private async handleSseEvent(ev: BpmnIqSseEvent): Promise<void> {
        ...
        try {
            ...
            await this.writeFromRemote(this.folder, ev.modelRef.relPath, full.xml, full.sha256);
            this.bumpModelCount();
        } catch (err) {
            this.vsUI.logError(err as Error);
        }
    ...
    private decodeModelId(id: string): { workspaceId: string; relPath: string } | null {
    ...
    private async writeFromRemote(
        ...
        const openDoc = workspace.textDocuments.find((d) => d.uri.toString() === uri.toString());
        if (openDoc) {
    ...
    private async removeFromRemote(folder: WorkspaceFolder, relPath: string): Promise<void> {
        if (!isSafeRelPath(relPath)) return;
        ...
        try {
            ...
            this.vsUI.logInfo(`[bpmn-iq] delete failed for ${relPath}: ${(err as Error).message}`);
        }
    }
