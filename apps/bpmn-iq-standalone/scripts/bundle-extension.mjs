#!/usr/bin/env node
// Copies the freshly-built bpmn-iq-vscode.vsix into ./plugins/ AND
...
//   corepack yarn build
//   (cd dist/apps/bpmn-iq-vscode && npx @vscode/vsce package --out bpmn-iq-vscode.vsix --yarn --no-dependencies)
//   corepack yarn workspace @miragon/bpmn-iq-standalone bundle
...
const vsixSrc = resolve(repoRoot, "dist", "apps", "bpmn-iq-vscode", "bpmn-iq-vscode.vsix");
const pluginsDir = resolve(standaloneDir, "plugins");
const pluginName = "bpmn-iq-vscode";
