...
export function mountDemoHeader(pageType: ModelType, links: Partial<DemoHeaderLinks> = {}): void {
    const l = { ...DEFAULT_LINKS, ...links };
    const active = getActiveModel(pageType);
    ...
    const options = MODELS.map(
        (m) =>
            `<option value="${m.id}"${m.id === active.id ? " selected" : ""}>` +
